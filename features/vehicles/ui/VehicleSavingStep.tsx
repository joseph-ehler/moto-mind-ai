...
export interface VehicleSavingStepProps {
  vehicleName: string;
  garageName?: string;
  onComplete: () => void;
  steps?: Step[];
...
const DEFAULT_STEPS: Step[] = [
  {
    icon: Save,
    text: "Saving vehicle profile…",
    subtext: "Creating your digital twin",
    duration: 1400,
    color: "text-blue-500",
  },
  {
    icon: Database,
    text: "Storing specifications…",
    subtext: "Preserving every detail",
    duration: 1200,
    color: "text-purple-500",
  },
  {
    icon: Building2,
    text: "Assigning to garage…",
    subtext: "Finding the perfect home",
    duration: 1500,
    color: "text-green-500",
  },
  {
    icon: Zap,
    text: "Triggering enhancements…",
    subtext: "Awakening AI intelligence",
    duration: 1100,
    color: "text-yellow-500",
  },
  {
    icon: CheckCircle,
    text: "Vehicle added to garage!",
    subtext: "Welcome to your fleet",
    duration: 1400,
    color: "text-green-600",
  },
];

export function VehicleSavingStep({
  vehicleName,
  garageName,
  onComplete,
  steps = DEFAULT_STEPS,
  ...
  // Auto advance through steps - PREDICTABLE TIMING
  React.useEffect(() => {
        ...
        <CardContent className="relative p-10 text-center">
          ...
          <div className="mb-8">
            <div className="inline-flex items-center gap-2 rounded-full border border-gray-200/60 bg-white/80 px-4 py-2 shadow-sm backdrop-blur-sm">
              ...
              <span className="font-mono text-sm tracking-wider text-gray-700" aria-label="Vehicle">
                {vehicleName}
              </span>
              {garageName && (
                <>
                  ...
                  <span className="text-sm text-gray-700">{garageName}</span>
                </>
              )}
            </div>
          </div>
