-- Migration: Auth Magic Links (Correct Version)
-- Created: 2025-10-18
-- Purpose: Replace old magic_links tables with new auth_magic_links structure

-- Drop OLD tables (from previous attempt with wrong names)
...
-- Drop any leftover functions with different signatures
DROP FUNCTION IF EXISTS check_magic_link_rate_limit(TEXT, TEXT, INTEGER, INTEGER);

-- Now create NEW structure with correct names

-- Create auth_magic_links table
...
-- Create cleanup function (returns INTEGER for count)
...
-- Create auth_magic_link_rate_limits table
...
BEGIN
  ...
  AND table_name IN ('auth_magic_links', 'auth_magic_link_rate_limits');
  ...
  IF table_count = 2 AND function_count = 2 THEN
    ...
    RAISE NOTICE '   - Dropped old tables: magic_links, auth_rate_limits';
    RAISE NOTICE '   - Created new tables: auth_magic_links, auth_magic_link_rate_limits';
