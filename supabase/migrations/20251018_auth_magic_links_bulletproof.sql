-- Migration: Auth Magic Links (Bulletproof Clean Install)
-- Created: 2025-10-18
-- Purpose: Add magic link authentication support (email + phone)
-- This version explicitly drops everything to ensure clean install

-- Drop all indexes first
DROP INDEX IF EXISTS idx_magic_links_token_hash;
DROP INDEX IF EXISTS idx_magic_links_identifier;
DROP INDEX IF EXISTS idx_magic_links_expires;
DROP INDEX IF EXISTS idx_magic_links_created;
DROP INDEX IF EXISTS idx_rate_limits_identifier;
DROP INDEX IF EXISTS idx_rate_limits_window;

-- Drop policies
DROP POLICY IF EXISTS "Allow all operations on auth_magic_links" ON auth_magic_links;
DROP POLICY IF EXISTS "Allow all operations on rate_limits" ON auth_magic_link_rate_limits;
...
CREATE OR REPLACE FUNCTION check_magic_link_rate_limit(
  p_identifier TEXT,
  p_method TEXT,
