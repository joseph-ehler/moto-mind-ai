-- Migration: Auth Magic Links (Clean Install)
-- Created: 2025-10-18
-- Purpose: Add magic link authentication support (email + phone)
-- This version drops existing objects first for clean reinstall
...
CREATE TABLE auth_magic_links (
  ...
  used_at TIMESTAMPTZ,
  metadata JSONB DEFAULT '{}'::jsonb  -- Store additional context (device, IP, etc.)
);
...
-- Permissive policy (auth handled in API via service role)
...
-- Create rate limiting table for magic link sends
...
DO $$ 
BEGIN 
  RAISE NOTICE '✅ Magic links tables and functions created successfully!';
END $$;
