-- MotoMindAI: Performance Indexes & Data Constraints (Base Tables)
-- Essential for production performance and data integrity

-- Performance indexes for tenant-scoped queries (base tables only)
...
-- Data validation constraints (base tables only)
ALTER TABLE vehicle_metrics
  ADD CONSTRAINT brake_wear_pct_range 
    CHECK (brake_wear_pct BETWEEN 0 AND 100 OR brake_wear_pct IS NULL),
  ADD CONSTRAINT data_completeness_pct_range 
    CHECK (data_completeness_pct BETWEEN 0 AND 100),
  ADD CONSTRAINT fuel_efficiency_positive 
    CHECK (fuel_efficiency_mpg > 0 OR fuel_efficiency_mpg IS NULL),
  ADD CONSTRAINT harsh_events_non_negative 
    CHECK (harsh_events >= 0),
  ADD CONSTRAINT idle_minutes_non_negative 
    CHECK (idle_minutes >= 0),
  ADD CONSTRAINT miles_driven_non_negative 
    CHECK (miles_driven >= 0 OR miles_driven IS NULL);

ALTER TABLE explanations
  ADD CONSTRAINT confidence_enum 
    CHECK (confidence IN ('high','medium','low')),
  ADD CONSTRAINT data_quality_score_range 
    CHECK (data_quality_score BETWEEN 0 AND 100 OR data_quality_score IS NULL);

-- Tenant name constraints
ALTER TABLE tenants
  ADD CONSTRAINT tenant_name_not_empty 
    CHECK (length(trim(name)) > 0),
  ADD CONSTRAINT tenant_kind_enum 
    CHECK (kind IN ('solo', 'org'));

-- Vehicle label constraints
ALTER TABLE vehicles
  ADD CONSTRAINT vehicle_label_not_empty 
    CHECK (length(trim(label)) > 0);

-- Membership role constraints
ALTER TABLE memberships
  ADD CONSTRAINT membership_role_enum 
    CHECK (role IN ('owner', 'manager', 'viewer'));

-- Provider integration constraints
ALTER TABLE provider_integrations
  ADD CONSTRAINT provider_name_enum 
    CHECK (provider IN ('samsara', 'geotab', 'fleet_complete'));

-- Create simple indexes for common query patterns
CREATE INDEX IF NOT EXISTS vehicles_updated_idx ON vehicles (tenant_id, updated_at DESC);
CREATE INDEX IF NOT EXISTS explanations_created_idx ON explanations (tenant_id, created_at DESC);
