...
async function updateRealTimeMetrics() {
  try {
    ...
    circuitBreakerState.set(
      ...
      circuitStatus.global.isOpen ? 1 : 0
    )
    
    // Per-tenant circuit breakers
    if (circuitStatus.openTenants) {
      circuitStatus.openTenants.forEach(tenantId => {
        circuitBreakerState.set(
...
// Helper function to record explanation metrics (used by explain API)
